...
function LinkTag({
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        'flex-center size-8 rounded-[4px] border border-transparent bg-transparent text-black/40 hover:cursor-pointer hover:bg-[#2C2D36]',
        className,
      )}
    >
      {children}
  ...
  if (type === 'page') {
    return (
      <LinkTag
        href={route}
        title={page.toString()}
        className={page === currentPage ? 'border-white bg-[#2C2D36] text-black' : ''}
      >
        {page}
      </LinkTag>
    );
